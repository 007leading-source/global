...
await send({action:"restock",ProductID:id,QuantityAdded:q,Cost:c});
loadProducts();
}
...
const itemsEncoded=encodeURIComponent(JSON.stringify(cartItems));

try{
const res=await fetch(API+`?action=createSale&CustomerID=${customerSelect.value}&Subtotal=${subtotal.innerText}&Discount=${discount.value||0}&IVA=${iva.innerText}&Total=${total.innerText}&items=${itemsEncoded}`);

const data=await res.json();

/* 🔥 FIX: Ensure items exist */
if(!data.items){
data.items = cartItems.map(i=>({
...
doc.text("Factura "+(d.invoice||"N/A"),10,10);
...
doc.text("Total: $"+(d.total||0),10,y+10);

doc.save("Factura_"+(d.invoice||Date.now())+".pdf");
}